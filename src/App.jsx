...
const T = {
  en: {
    ...
    genuineNote:"💡 Note: Genuine bank messages from official sender IDs like VM-SBIINB, AD-HDFCBK, BW-ICICIB are usually safe. Our AI recognises these.",
  },
  hi: {
    ...
    genuineNote:"💡 नोट: VM-SBIINB, AD-HDFCBK जैसे आधिकारिक Sender ID से आने वाले बैंक SMS आमतौर पर असली होते हैं।",
  },
  bn: {
...
CRITICAL RULES:
1. Indian bank SMS from official sender IDs (VM-SBIINB, AD-HDFCBK, BW-ICICIB, TM-AXISBK, JD-KOTAKB, CP-PNBSMS, IM-CANBNK etc.) are GENUINE transaction alerts — mark as LEGITIMATE
2. OTP messages from official bank sender IDs are GENUINE — mark as LEGITIMATE  
3. Only flag as SCAM if there are clear red flags: requests for OTP/PIN/password, suspicious links, fake urgency, requests to call unofficial numbers, requests for money transfer
4. SUSPICIOUS means some elements look unusual but not clearly fraudulent
5. Write your reasoning in simple language that a non-technical person can understand
6. The user may submit messages in any Indian or international language — respond in English regardless
Respond ONLY in valid JSON (no markdown):
{"verdict":"LEGITIMATE"|"SUSPICIOUS"|"SCAM","scamType":"type or null","riskScore":0-100,"summary":"clear one sentence for a general user","reasoning":"3-4 sentences in simple language explaining why this is legitimate/suspicious/scam","redFlags":["specific red flag found"],"safeIndicators":["specific genuine indicator found"],"recommendation":"exactly what the user should do — if LEGITIMATE say it's safe but remind them never to share OTP with anyone"}`;
